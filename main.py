...
class PhoneBook:
    ...
    def show_contacts(self):
        ...
        try:
            ...
            if not search:
                ...
                self.cursor.execute(
                    f"SELECT * FROM CONTACTS_TABLE WHERE Mobile LIKE '%{search}%' OR NAME LIKE '%{search}%'"
                )
    ...
    def edit_contact(self):
        if not self.tree.selection():
            ...
            self.cursor.execute(
                f"SELECT * FROM CONTACTS_TABLE WHERE Mobile = {selected_item[1]} OR Mobile = '0{selected_item[1]}'"
            )
    ...
    def delete_contact(self):
        if not self.tree.selection():
            ...
            if result == "yes":
                ...
                self.cursor.execute(
                    f"DELETE FROM CONTACTS_TABLE WHERE Mobile = {selecteditem[1]} OR Mobile = '0{selecteditem[1]}'"
                )
                self.con.commit()
                self.show_contacts()
    ...
    def edit_contact_in_db(self, mobile):
        ...
        self.cursor.execute(
            f"DELETE FROM CONTACTS_TABLE WHERE Mobile = {mobile} OR Mobile = '0{mobile}'"
        )
    ...
    def save_contact_to_db(self):
        ...
        if (
            ...
            self.cursor.execute(
                f"SELECT * FROM CONTACTS_TABLE WHERE Mobile = {mobile} OR Mobile = '0{mobile}'"
            )
            res = self.cursor.fetchall()
            if len(res) == 0:
                self.cursor.execute(
                    f"""INSERT INTO CONTACTS_TABLE VALUES(
                                '{name}','{mobile}','{Work}','{Birthday}','{email}','{notes}')
                                """
                )
                self.con.commit()
    ...
    def main_page_gui(self):
        ...
        self.style.configure(
            "Treeview.Heading", background="#fce1e4", font=("Arial", 10)
        )
        ...
        self.tree.heading("3", text="Work")
        self.tree.heading("4", text="Birthday")
    ...
    def edit_contact_gui(self, contact_data):
        ...
        self.Work.set(contact_data[2])
        self.Birthday = tk.StringVar()
        self.Birthday.set(contact_data[3])
